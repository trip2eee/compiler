...
Figure 5.5 The DFA of sets of LR(0) items for Example 5.9
```mermaid
graph LR
...
We say that a grammar is an SLR(1)grammar if the application of the above SLR(1) parsing rules results in no ambiguity. In particular, a grammar is SLR(1) if and only if , for any state $s$, the following two conditions are satisfied:
...
- A violation of the first condition: *shift-reduce conflict*
- A violation of the second condition: *reduce-reduce conflict*

#### Example 5.10

Consider grammar of Example 5.8, whose DFA of sets of items is given in Figure 5.4.
...
- General LR(1) parsing
  - Sometimes called canonical LR(1) parsing
  - Considered too complex to use in the construction of parsers in most situations.

- LALR(1)
  - Lookahead LR parsing
  - A modification of general LR(1) parsing

### 5.4.1 Finite Automata of LR(1) Items
SLR(1) applies lookaheads after the construction of the DFA of LR(0) items, a construction that ignores lookaheads.

LR(1) method construct DFA that has the lookaheads built into its construction from the start. This DFAuses items that are an extension of LR(0) items. They are called LR(1) items.

We write LR(1) items using square brackets as 

$[A \rightarrow \alpha . \beta , a]$

where $A \rightarrow \alpha .\beta$ is an LR(0) item and $a$ is a token (the lookahead).

#### Definition
*Definition of LR(1) transitions.*

1. Given an LR(1) items $[A \rightarrow \alpha . X \gamma, a]$, where $X$ is any symbol (terminal or nonterminal), there is a transition on $X$ to the item $[A \rightarrow \alpha X. \gamma, a]$

2. Given an LR(1) items $[A \rightarrow \alpha . B \gamma, a]$, where $B$ is a nonterminal, there are $\epsilon$-transitions to items $[B \rightarrow .\beta, b]$ for every production $B \rightarrow \beta$ and for every token $b$ in $First(\gamma a)$

#### Example 5.14

Consider the grammar $A \rightarrow ( \ A \ ) \ \mid \ a$ of Example 5.9.

We begin the construction of its DFA of sets of LR(1) items by augmenting the grammar and forming the initial LR(1) item $[A' \rightarrow .A, \ \$]$. The $\epsilon$-closure of this item is the start state of the DFA.

$\epsilon$-transitions to items $[A \rightarrow . ( \ A \ ), \ \$]$ and $[A \rightarrow .a, \ \$]$

The start state (state 0) is then the set of theese three items
...
There is a transition on $A$ to the closure of the containing the item $[A' \rightarrow A., \ \$]$

State 1:

$[A' \rightarrow A., \$]$

Transition on the token "(" to the closure of the set consisting of the item $[A \rightarrow (. \ A \ ), \ \$]$. There are $\epsilon$-transitions from this item, $[A \rightarrow .( \ A \ ), \ )]$ and $[A \rightarrow .a, \ )]$
...
We return once more to state 0, where we find a last transition to the sate generated by the item $[A \rightarrow a., \ \$]$

State 3:

$[A \rightarrow a., \ \$]$

We return to state 2. From this state there is a transition on $A$ to the $\epsilon$-closure of $[A \rightarrow ( \ A \ .), \ \$]$
...
Figure 5.7 The DFA of sets of LR(1) items for Example 5.14

```mermaid
graph LR
S0["(0)<br>[A' -> .A, $]<br>[A -> .( A ), $]<br>[A ->.a, $]"]
S1["(1)<br>[A' -> A., $]"]
S2["(2)<br>[A -> ( .A ), $]<br>[A -> .( A ), )]<br>[A -> .a, )]"]
S3["(3)<br>[A -> a., $]"]
S4["(4)<br>[A -> ( A. ), $"]
S5["(5)<br>[A -> ( .A ), )]<br>[A -> .( A ), )]<br>[A -> .a, )]"]
S6["(6)<br>[A -> a., )]"]
S7["(7)<br>[A -> ( A )., $]"]
S8["(8)<br>[A -> ( A. ), )]"]
S9["(9)<br>[A -> ( A )., )]"]
...
1. If state $s$ contains any LR(1) item of the form $[A \rightarrow \alpha. X \beta, a]$, where $X$ is a terminal, and $X$ is the next token in the input string
   - the action is to shift the current input token onto the stack
   - the new state to be pushed on the stack is the state containing LR(1) item $[A \rightarrow \alpha X. \beta, a]$

2. If state $s$ contains the complete LR(1) item $[A \rightarrow \alpha ., a]$
 - if the next token in the input string is $a$
   - the action is to reduce by the rule $A \rightarrow \alpha$.
   - a reduction by the rule $S' \rightarrow S$, where $S$ is the start state, is equivalent to acceptance.

 - In other cases, the new state is computed as follows
   - Remove the string $\alpha$ and all of its corresponding states from the parsing stack.
   - Correspondingly, back up in the DFA to the state from which the construction of $\alpha$ began.
   - By construction, this state must contain an LR(1) item of the form $[B \rightarrow \alpha . A \beta, b]$. Push $A$ onto the stack, and push the state containing the item $[B \rightarrow \alpha A . \beta, b]$

3. If the next input token is such that neither of the above two cases applies, an error is declared.


### 5.4.3 LALR(1) Parsing
In LR(1), there are many LR(1) items differing only in the lookahead symbols.

The LALR(1) parsing algorithm expresses the fact that it makes sense to identify all such states and combine their lookaheads. In doing so, we always must end up with a DFA that is identical to the DFA of LR(0) items, except that each state consists of items with sets of lookaheads.


Principles of LALR(1) Parsing
1. The core of a state of the DFA of LR(1) items is a state of the DFA of LR(0) items.
2. Given two states $s_1$ and $s_2$ of the DFA of LR(1) items that have the same core, suppose there is a transition on the symbol $X$ from $s_1$ to a state $t_1$. Then there is also a transition on $X$ from state $s_2$ to a state $t_2$, and the states $t_1$ and $t_2$ have the same core.

These two principles allow us to construct the DFA of LALR(1) items, which is constructed from the DFA of LR(1) items by identifying all states that have the same core and forming the union of the lookahead symbols for each LR(0) item. Thus, each LALR(1) item in this DFA will have an LR(0) item as its first component and a set of lookahead tokens as its second component.

We will denote multiple lookaheads by writing a / between them.

Thus, the LALR(1) item $[A \rightarrow \alpha . \beta, \ a / b / c]$ has a lookahead set consisting of the symbols $a$, $b$, and $c$.


#### Example 5.17
Consider the grammar of Example 5.14

$A \rightarrow ( \ A \ ) \ \mid \ a$

Figure 5.9 The DFA of sets of LALR(1) items for Example 5.17
```mermaid
graph LR
S0["(0)<br>[A' -> .A, $]<br>[A -> .( A ), $]<br>[A -> .a, $]"]
S1["(1)<br>[A' -> A., $]"]
...
S4 -- ")" --> S5
```
where $ is the endmarker
The $ lookahead is said to be spontaneously generated.
